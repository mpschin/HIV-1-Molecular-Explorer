...
if "phylo_result" not in st.session_state:
    st.session_state.phylo_result = None
...
# ─── Header ───────────────────────────────────────────────────────────────────
st.markdown('<div class="main-header"><span>NCBI</span> HIV-1 Molecular Explorer </div>', unsafe_allow_html=True)
...
def compute_pairwise_distance(seq1: str, seq2: str) -> float:
    """p-distance between two aligned sequences (gap columns excluded)."""
...
def build_phylo_tree(aligned_seqs: list):
    """
    Build a Neighbor-Joining phylogenetic tree from MSA output.
    aligned_seqs: list of (genbank_id, aligned_sequence) tuples.
    Returns (Bio.Phylo tree, newick_string).
    """
    from Bio.Phylo.TreeConstruction import DistanceMatrix, DistanceTreeConstructor
    from Bio import Phylo as BioPhylo

    names = [sid for sid, _ in aligned_seqs]
    seqs  = [seq  for _, seq  in aligned_seqs]
    n = len(names)

    # Lower-triangular distance matrix required by Biopython
    matrix = []
    for i in range(n):
        row = [compute_pairwise_distance(seqs[i], seqs[j]) for j in range(i)] + [0.0]
        matrix.append(row)
...
def plot_phylo_tree(tree, meta: dict) -> go.Figure:
    """
    Render a Biopython tree as an interactive Plotly rectangular cladogram.
    meta: {leaf_name: {"country": str, "year": str}}
    """
    xs: dict = {}
    ys: dict = {}
    leaf_counter = [0]
    ...
    def _layout(clade, parent_x=0.0):
        x = parent_x + (clade.branch_length or 0.0)
        xs[id(clade)] = x
        if clade.is_terminal():
            ys[id(clade)] = leaf_counter[0]
            leaf_counter[0] += 1
        else:
            for child in clade.clades:
                _layout(child, x)
            ys[id(clade)] = sum(ys[id(c)] for c in clade.clades) / len(clade.clades)

    _layout(tree.root)

    # Normalise x to [0, 1]
    max_x = max(xs.values()) or 1.0
    xs = {k: v / max_x for k, v in xs.items()}

    # ── 2. Country colour palette ────────────────────────────────────────────
    PALETTE = [
        "#00C2CB", "#F6AD55", "#68D391", "#FC8181", "#63B3ED",
        "#B794F4", "#F687B3", "#FFD740", "#4FD1C5", "#A0AEC0",
    ]
    country_colors: dict = {}
    color_idx = [0]

    def _color(country: str) -> str:
        if country not in country_colors:
            country_colors[country] = PALETTE[color_idx[0] % len(PALETTE)]
            color_idx[0] += 1
        return country_colors[country]

    # ── 3. Build edge traces and leaf data ───────────────────────────────────
    edge_x, edge_y = [], []
    leaf_by_country: dict = {}

    def _draw(clade):
        cx, cy = xs[id(clade)], ys[id(clade)]

        if not clade.is_terminal():
            child_ys = [ys[id(c)] for c in clade.clades]
            # Vertical connector
            edge_x.extend([cx, cx, None])
            edge_y.extend([min(child_ys), max(child_ys), None])
            for child in clade.clades:
                chx, chy = xs[id(child)], ys[id(child)]
                # Horizontal branch
                edge_x.extend([cx, chx, None])
                edge_y.extend([chy, chy, None])
                _draw(child)
        else:
            name = clade.name or ""
            info = meta.get(name, {})
            ...
            year    = info.get("year", "")
            color   = _color(country)
            hover   = f"<b>{name}</b><br>Country: {country}<br>Year: {year}"

            bucket = leaf_by_country.setdefault(
                country, {"x": [], "y": [], "labels": [], "hover": [], "color": color}
            )
            bucket["x"].append(cx)
            bucket["y"].append(cy)
            bucket["labels"].append(name[:16])
    ...
    for country, data in leaf_by_country.items():
        traces.append(go.Scatter(
            x=data["x"], y=data["y"],
            mode="markers+text",
            marker=dict(size=9, color=data["color"], symbol="circle",
                        line=dict(color="#111827", width=1)),
            text=data["labels"],
            textposition="middle right",
            textfont=dict(size=10, color="#B0BAD0", family="Courier New, monospace"),
    ...
    fig.update_layout(
        ...
        height=max(420, n_leaves * 34),
        margin=dict(l=10, r=200, t=30, b=30),
        xaxis=dict(
            title="Relative Branch Length",
            range=[-0.02, 1.35],
            showgrid=False, zeroline=False, showline=False,
            tickfont=dict(color="#6E7891", size=10),
        ),
        yaxis=dict(visible=False, range=[-0.8, n_leaves - 0.2]),
        legend=dict(
            title=dict(text="Country", font=dict(color="#9AA5C0")),
            font=dict(size=11, color="#B0BAD0"),
            bgcolor="rgba(26,31,46,0.92)",
            bordercolor="#252D40", borderwidth=1,
